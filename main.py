...
def send_welcome(message):
    database.create_table_in_db_by_userid(message.chat.id)
...
def send_lowprice(message):
    user_input_dct[f'user_{message.chat.id}'] = {}
    with open(file='logs_command.log', mode='a', encoding='UTF-8') as logfile:
    ...
    user_input_dct[f'user_{message.chat.id}']['command_flag'] = 'lowprice'
    first_step(message)
...
def send_highprice(message):
    ...
    user_input_dct[f'user_{message.chat.id}']['command_flag'] = 'highprice'
    first_step(message)
...
def send_bestdeal(message):
    ...
    user_input_dct[f'user_{message.chat.id}']['command_flag'] = 'bestdeal'
...
def send_history(message):
    ...
    history_for_send = database.print_db_info(message.chat.id)
...
def city_reg(message):
    user_input_dct[f'user_{message.chat.id}']['date_flag'] = False
    user_input_dct[f'user_{message.chat.id}']['date_range'] = []
    user_input_dct[f'user_{message.chat.id}']['city'] = message.text
    check_in = 'Введите дату заезда: '
    bot.send_message(message.chat.id, check_in)
    input_by_calend_buttons(message)
    while not user_input_dct[f'user_{message.chat.id}']['date_flag']:
        pass
    check_in_f(message)
...
def check_out_f(message):
    user_input_dct[f'user_{message.chat.id}']['date_difference'] = (user_input_dct[f'user_{message.chat.id}']['date_range'][1] - user_input_dct[f'user_{message.chat.id}']['date_range'][0]).days
...
def num_hotels_f(message):
    if message.text.isdigit():
        if int(message.text) > 15:
            ...
            user_input_dct[f'user_{message.chat.id}']['num_hotels'] = 15
        elif int(message.text) <= 15:
            user_input_dct[f'user_{message.chat.id}']['num_hotels'] = int(message.text)
...
def send_photo_f(message):
    if message.text.lower() == 'да':
        user_input_dct[f'user_{message.chat.id}']['send_photo'] = True
        ...
        user_input_dct[f'user_{message.chat.id}']['send_photo'] = False
...
def num_hot_photo_f(message):
    if int(message.text) > 15:
        bot.send_message(message.chat.id, 'Слишком большая цифра. Мой лимит 15. Столько и выведу.')
        user_input_dct[f'user_{message.chat.id}']['num_hot_photo'] = 15
    else:
        user_input_dct[f'user_{message.chat.id}']['num_hot_photo'] = int(message.text)
        req_and_answer(message)
...
def sort_results(message, hotels_lst):
    ...
    if user_input_dct[f'user_{message.chat.id}']['command_flag'] == 'lowprice':
        hotels_lst = sorted(hotels_lst, key=lambda i_hotel: i_hotel['ratePlan']['price']['current'])
    elif user_input_dct[f'user_{message.chat.id}']['command_flag'] == 'highprice':
...
def bestdeal_func(i_hotel, message):
    ...
    if (user_input_dct[f'user_{message.chat.id}']['min_price'] <= cur_price <= user_input_dct[f'user_{message.chat.id}']['max_price']) \
            and user_input_dct[f'user_{message.chat.id}']['min_distance'] <= cur_distance <= user_input_dct[f'user_{message.chat.id}']['max_distance']:
        return i_hotel
...
def print_info(message, hotels_lst):
    ...
    bot.send_message(message.chat.id, f'{user_input_dct[f"user_{message.chat.id}"]["city"]}')
    its_already_history = ''.join([its_already_history, f'{user_input_dct[f"user_{message.chat.id}"]["city"]}', '\n'])

    for i_num in range(user_input_dct[f"user_{message.chat.id}"]['num_hotels']):
        ...
        try:
            ...
            logger.info(f'Пользователь id {message.chat.id}|{error}')
    else:
        bot.send_message(message.chat.id, 'Вывод окончен. Надеюсь, вы нашли то, что вам нужно!')
    database.add_data_to_db(message.chat.id, its_already_history)
...
def max_price_f(message):
    ...
    user_input_dct[f"user_{message.chat.id}"]['min_price'] = users_price
...
def min_distance_f(message):
    ...
    if len(users_price) == 2:
        ...
        users_price = float(users_price[0])
    user_input_dct[f"user_{message.chat.id}"]['max_price'] = users_price
...
def max_distance_f(message):
    ...
    user_input_dct[f"user_{message.chat.id}"]['min_distance'] = round(users_distance / 1609.34, 2)
...
def from_bestdeal_to_common_struct(message):
    users_distance = float(re.search(r'\d+', message.text).group(0))
    user_input_dct[f"user_{message.chat.id}"]['max_distance'] = round(users_distance / 1609.34, 2)
    first_step(message)
...
def cal1(c):
    ...
    if not result and key:
        ...
        user_input_dct[f"user_{c.message.chat.id}"]['date_flag'] = True
        user_input_dct[f"user_{c.message.chat.id}"]['date_range'].append(result)
        bot.edit_message_text(f"Ваш выбор: {result} ",
                              c.message.chat.id,
                              c.message.message_id)


bot.infinity_polling()  #Постоянная проверка ботом чата
